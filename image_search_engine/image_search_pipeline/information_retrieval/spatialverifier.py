...
class SpatialVerifier:
    ...
    def rerank(self, queryKps, queryDescs, searchResult, numResults = 10):
        ...
        for (i, (start, end)) in zip(resultIdxs, self.featuresDB["index"][resultsIdx, ...]):
            ...
            return SearchResult(results[:numResults], (datetime.datetime.now() - startTime).to_second())

        def match(self, kpsA, featuresA, kpsB, featuresB):
            # compute the raw matches and initialize the list of actual matches
            # and the list of inlier indexes.
            matcher = DescriptorExtractor_create("BruteForce")
            rawMatches = matcher.knnMatch(featuresB, feturesA, 2)
            matches = []
            inlierIdxs = None
            ...
            if len(matches) >= self.minMatches:
                ...
                if idxs.shape[0] == 0:
                    return None
                inlierIdxs = pairwise.euclidean_distance(featuresB[idx], Y = self.vocab)
